...
void mandelbrot::output_image(const char *filename)
{
        ...
        std::uint8_t header[18] = {
                ...
                0,
                0,
                0, // Empty colour map specification
                0,
                0, // X origin
                0,
                0, // Y origin
                static_cast<std::uint8_t>(this->width & 0xFF),
                static_cast<std::uint8_t>((this->width >> 8) & 0xFF), // Width
                static_cast<std::uint8_t>(this->height & 0xff),
                static_cast<std::uint8_t>((this->height >> 8) & 0xff), // Height
                24, // Bits per pixel
                0, // Image descriptor
        };
...
/* @start Pixel each thread should begin at
 * @strip How many pixels to compute per thread
 * Computes the mandelbrot set and
 * assignes a colour value to each pixel based on
 * number of iterations taken
 */
void mandelbrot::compute_mandelbrot(int start, int strip)
{
        for (int i = start; i < start + strip; ++i) {
                // Get current x/y value since treating 1d array as 2d
                int x = (int)(i % this->width);
                int y = (int)(i / this->width);

                std::complex<double> complex_coords(
                        (x - (this->scale * this->width)) / (this->width / 4.0),
...
void mandelbrot::create_mandelbrot()
{
        ...
        for (int i = 0; i < chunkNum; ++i) {
                ...
                things.push_back(std::thread(&mandelbrot::compute_mandelbrot,
